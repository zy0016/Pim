...
int ME_GSMInit_SIM( HWND hWnd, UINT Msg )
{
    ...
    /* Fill the request */
	request->request.cmd = ME_INIT_WAVE_SIM_SET;
    request->request.reply = ME_WndCallback;
...
int ME_GSMInit_Reset (HWND hWnd, UINT Msg)
{
	int retval = 0;
	struct ME_REQUEST_WIN * request = FindRequestStruct( -1 );
...
int ME_SMS_SetFormat( HWND hwnd, int msg, BOOL bPDU )
{
    ...
    /* Fill the request */
	request->request.arg1 = bPDU;
	request->request.cmd = ME_SMS_FORMAT;
    request->request.reply = ME_WndCallback;
...
int ME_SMS_SetProtocol( HWND hwnd, int msg, BOOL bPhasePlus )
{
    ...
    /* Fill the request */
	request->request.arg1 = bPhasePlus;
	request->request.cmd = ME_SMS_PHASE;
    request->request.reply = ME_WndCallback;
...
{
	return GetSTKMainMenu( buf, buflen );
}
...
int ME_DisableSIMCard (HWND hwnd, int msg)
{
    ...
    /* Fill the request */
	request->request.cmd = ME_DISABLE_SIM;
    request->request.reply = ME_WndCallback;
...
int ME_SetVBDR (HWND hwnd, int msg)
{
    ...
    /* Fill the request */
	request->request.cmd = ME_SET_VBDR;
    request->request.reply = ME_WndCallback;
...
int ME_SetAudioMode (HWND hwnd, int msg, BOOL bEarPhone)
{
    ...
    request->hWnd = hwnd;
    request->message = msg;
	request->request.arg1 = bEarPhone;

    /* Fill the request */
	request->request.cmd = ME_EAR_AUDIO;
    request->request.reply = ME_WndCallback;
...
int ME_PhonebookReady (HWND hwnd, int msg)
{
    ...
    /* Fill the request */
	request->request.cmd = ME_PB_READY;
    request->request.reply = ME_WndCallback;
