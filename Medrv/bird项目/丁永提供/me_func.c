...
static MEREQUEST * gReqUssd;
static struct ME_INDICATION * current_ind; 

extern int MAT_Callback (void);
...
static int FSM_Entry (MEREQUEST * req, int event, char * data, int datalen);

static int FSM_USSD             (int event, char* data, int datalen);
static int FSM_SpecialCall      (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SetCallFwding    (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_GetCallFwding    (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SetCallBarring   (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_GetCallBarring   (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SMSSend          (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SMSRead          (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SMSList          (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SMSDelete        (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_PBookRead        (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_SetPhoneLock     (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_GetPhoneLock     (MEREQUEST * req, int event, char* data, int datalen);
static int FSM_ChangePassword   (MEREQUEST * req, int event, char* data, int datalen);
...
static BOOL MultiLinesURC (int index);

void       GetResultLength (MEREQUEST* pRequest);
static int GetResultErrorCode (MEREQUEST * req, char* data, int datalen);
...
#define TOPVOLTAGE     4200
#define BASEVOLTAGE    3400
...
static int me_init (MEREQUEST * pRequest)
{
    ...
    if ( MAT_Init (pRequest->arg1, 1, 0, ME_MatIndication) < 0 || 
         MAT_Init (pRequest->arg1, 2, 0, ME_MatIndication1) < 0 )
        return -1;

    memset (gQueue, 0, sizeof(gQueue));
    gQueue[0].entry = gQueue[1].entry = FSM_Entry;
...
{
    if ( MAT_Init (pRequest->arg1, 0, 0, ME_UssdIndication) < 0 )
        return -1;
...
int ME_Request (MEREQUEST * pRequest)
{
    ...
    if ( !IsAsyncCmd (pRequest->cmd) )
    {
        ...
        int id = GetQueue (pRequest->cmd);
        pRequest->queue = id;
        if ( gQueue[id].Head != NULL ) {
            /* We are in command exec, add command into list tail */
            gQueue[id].Tail->next = pRequest;
            gQueue[id].Tail = pRequest;
        }
        else {
            /* We are not in command exec */
            gQueue[id].Head = gQueue[id].Tail = pRequest;
            /* Start do this command */
            gQueue[id].Head->stat = FSM_STATE_INIT;
            PostMessage (ME_hWnd, WM_FSM_START, pRequest->queue, 0);
        }
...
{
    int id = pRequest->queue;
    if ( pRequest->index == gQueue[id].Head->index ) {
        gQueue[id].entry (pRequest, USERSTOP, 0, 0);
        if ( gQueue[id].Head != NULL && gQueue[id].Head->stat == FSM_STATE_DONE ) {
            if ( gQueue[id].Head->reply )
                gQueue[id].Head->reply (gQueue[id].Head);

            gQueue[id].Head = gQueue[id].Head->next;

            if ( gQueue[id].Head ) {
                gQueue[id].Head->stat = FSM_STATE_INIT;
                PostMessage (ME_hWnd, WM_FSM_START, gQueue[id].Head->queue, 0);
            }
        }
    }
    else {
        MEREQUEST *cur_req, *pri_req;
        pri_req = gQueue[id].Head;
        cur_req = gQueue[id].Head->next;
        while ( cur_req != NULL ) {
...
int ME_GetCurrentResult (void* result, int result_len)
{
    ...
    // 当前结果为请求返回结果
    if ( gQueue[0].Head != NULL && gQueue[0].Head->stat == FSM_STATE_DONE )
        req = gQueue[0].Head;
    else if ( gQueue[1].Head != NULL && gQueue[1].Head->stat == FSM_STATE_DONE )
        req = gQueue[1].Head;

    if ( req != NULL ) {
        if ( NULL == result )
            return req->result_len;

        if ( result_len < 0 || req->result_len < 0 )
            return -1;

        if ( result_len < req->result_len )
            return -1;

        return FillResultField (result, result_len, req);
    }
...
static int OnATDataIn (void * pdata, int datalen)
{
    ...
    for ( i = 0; i < datalen; i++, ptr2++ ) {
        ...
        if ( *ptr2 != '\n' ) {
            ...
            if ( (*ptr2 == '>') && (ptr2 == ptr1) ) {
                char escape = 0x1b;
                if ( gQueue[0].Head != NULL ) {
                    retval = gQueue[0].entry (gQueue[0].Head, DATAIN, ptr1, 1);
        ...
        *ptr2 = '\0';
        ...
        if ( gQueue[0].Head != NULL || !gInitDone ) {
            retval = gQueue[0].entry (gQueue[0].Head, DATAIN, ptr1, ptr2-ptr1+1);
...
check_cmdqueue:
        ...
        {
            /* Call reply function */
            if ( gQueue[0].Head->reply )
            {
                gQueue[0].Head->reply (gQueue[0].Head);
                ME_MemInitial (0);
            }
            /* Unlink it from list */
            gQueue[0].Head = gQueue[0].Head->next;
            
            /* Check for start next command */
            if ( gQueue[0].Head )
            {
                /* Start do this command */
                gQueue[0].Head->stat = FSM_STATE_INIT;
...
static int OnATDataIn1 (void * pdata, int datalen)
{
    ...
    for ( i = 0; i < datalen; i++, ptr2++ ) {
        ...
        if ( *ptr2 != '\n' ) {
            ...
            if ( (*ptr2 == '>') && (ptr2 == ptr1) ) {
                char escape = 0x1b;
                if ( gQueue[1].Head != NULL ) {
                    retval = gQueue[1].entry (gQueue[1].Head, DATAIN, ptr1, 1);
        ...
        *ptr2 = '\0';
        ...
        /* We here send data to state machine first */
        if ( gQueue[1].Head != NULL ) {
            retval = gQueue[1].entry (gQueue[1].Head, DATAIN, ptr1, ptr2-ptr1+1);
...
check_cmdqueue:
        if ( gQueue[1].Head != NULL && gQueue[1].Head->stat == FSM_STATE_DONE )
        {
            /* Call reply function */
            if ( gQueue[1].Head->reply )
            {
                gQueue[1].Head->reply (gQueue[1].Head);
                ME_MemInitial (1);
            }
            /* Unlink it from list */
            gQueue[1].Head = gQueue[1].Head->next;
            
            /* Check for start next command */
            if ( gQueue[1].Head )
            {
                /* Start do this command */
                gQueue[1].Head->stat = FSM_STATE_INIT;
                PostMessage (ME_hWnd, WM_FSM_START, 1, 0);
            }
        }
    }
...
LRESULT CALLBACK ME_WndProc( HWND hWnd, UINT wMsgCmd, WPARAM wParam, LPARAM lParam )
{
    ...
    {
        while ( gQueue[id].Head != NULL )
        {
            gQueue[id].Head->stat = FSM_STATE_INIT;
            gQueue[id].entry (gQueue[id].Head, START, 0, 0);
            if ( gQueue[id].Head->stat != FSM_STATE_DONE )
                break;
            {
                MEREQUEST * request = gQueue[id].Head;
                if ( request->reply )
                {
                    request->reply (request);
                    ME_MemInitial (id);
                }
                gQueue[id].Head = request->next;
            }
        }
    }
    else
...
void CALLBACK ME_TimerProc (HWND hWnd, UINT Msg, UINT id, DWORD Time)
{
    ...
    MsgOut ("\r\nME Request[%d] Timeout!", i+1);
#endif

    gQueue[i].entry (gQueue[i].Head, TIMEOUT, 0, 0);

    if ( gQueue[i].Head != NULL && gQueue[i].Head->stat == FSM_STATE_DONE ) {
        /* Call reply function */
        if ( gQueue[i].Head->reply )
        {
            gQueue[i].Head->reply (gQueue[i].Head);
            ME_MemInitial (i);
        }
        /* Unlink it from list */
        gQueue[i].Head = gQueue[i].Head->next;

        /* Check for start next command */
        if ( gQueue[i].Head != NULL )
        {
            /* Start do this command */
            gQueue[i].Head->stat = FSM_STATE_INIT;
            PostMessage (ME_hWnd, WM_FSM_START, i, 0);
        }
    }
}
...
static int FSM_Entry (MEREQUEST * req, int event, char * data, int datalen)
{
    ...
    case ME_INIT_EVERYTIME:
        ...
        retval = ME_FSM_Init (req, event, retval, data, datalen);
        break;

    case ME_GETLASTERROR:

        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CEER\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
                retval = GetResultErrorCode (req, data, datalen);

                if ( retval < 0 )
                    return retval;

                if ( !(req->error == ME_RS_SUCCESS || req->error == ME_RS_FAILURE))
                    return -1;

                KillTimer (NULL, gQueue[req->queue].TimerID);
                GetResultLength (req);
                req->stat = FSM_STATE_DONE;
            }
    ...
    case ME_STK_SEND:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                char* cmd = ((STK_PARAM*)(req->cmddata))->cmd;
                int  timeout = ((STK_PARAM*)(req->cmddata))->timeout;

                if ( cmd == NULL )
                    goto ERROR_HANDLER;

                retval = MAT_Send (req->queue+1, cmd, strlen(cmd));
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, timeout, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            static int index = 0;
            char* p = NULL;
            int count = ((STK_PARAM*)(req->cmddata))->line_count;
            int len = ((STK_PARAM*)(req->cmddata))->line_len;
            char* linebreak;
            ...
            if ( DATAIN == event )
            {
                ...
                {
                    data[datalen-1] = '\n';
                    p = (char*)(((STK_PARAM*)(req->cmddata))->revbuf) + index*len;
    ...
    case ME_STK_GETSAT:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^SSTA?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_SSCALL:
        ...
        return FSM_SpecialCall (req, event, data, datalen);
#if 0
    case ME_USSD:
    ...
    case ME_VOICE_DIAL:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                char* cmddata = req->cmddata;

                if ( req->cmddata_len == (int)strlen(cmddata) )
                    sprintf( ATCmd, "ATD%s;\r", cmddata );
                else
                    sprintf( ATCmd, "ATD%s+%s;\r", cmddata, &cmddata[strlen(cmddata)+1] );

                retval = MAT_Send (req->queue+1, ATCmd, strlen(ATCmd) );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, 120000, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                if ( retval != FRC_OK ) {
                    KillTimer (NULL, gQueue[req->queue].TimerID);
                    MAT_Send (req->queue+1, "AT+CEER\r", -1);
                    gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                    req->stat = FSM_STATE_STEP3;
                    return 0;
                }
            ...
            {
                retval = MAT_Send (req->queue+1, "B\r", strlen("B\r"));

                KillTimer (NULL, gQueue[req->queue].TimerID);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP2;
            }
            else if ( TIMEOUT == event )
                goto TIMEOUT_HANDLER;
        }
        else if ( FSM_STATE_STEP2 == req->stat )
        {
            if ( DATAIN == event )
            {
                retval = GetResultErrorCode (req, data, datalen);
                ...
                if ( retval == FRC_OK || retval == FRC_NOCARRIER )
                    req->error = ME_RS_USERSTOP;
                else
                    req->error = ME_RS_FAILURE;
        ...
        else if ( FSM_STATE_STEP3 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr (data, "+CEER:") == data )
                {
                    ...
                    lid = ME_GetIntSection (data, "+CEER:", 1);
                    reason = ME_GetIntSection (data, "+CEER:", 2);

                    if ( !(lid == 0 && reason == 0) ) {
                        req->errcode = MAKELONG(reason,lid);
                    }
    ...
    case ME_VOICE_HANGUP:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "ATH\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_VOICE_ANSWER:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "ATA\r", strlen("ATA\r") );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_CURRENT_CALLS:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CLCC\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                case CUT_ACTIVED_X:         // 挂断指定序号的激活通话
                    ...
                    if ( req->arg2 < 8 && req->arg2 > 0 )
                        sprintf( ATCmd, "AT+CHLD=1%d\r", req->arg2 );
                    else
                        goto ERROR_HANDLER;
                ...
                case ACTIVE_SWITCH_X:       // 将除指定序号外的所有通话变为保持
                    ...
                    if ( req->arg2 < 8 && req->arg2 > 0 )
                        sprintf( ATCmd, "AT+CHLD=2%d\r", req->arg2 );
                    else
                        goto ERROR_HANDLER;
    ...
    case ME_SET_AUTOANSWER:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                sprintf( ATCmd, "ATS0=%d\r", req->arg1 );
    ...
    case ME_GET_AUTOANSWER:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "ATS0?\r", strlen("ATS0?\r") );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_GET_CALLWAITING:
        ...
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr (data, "+CCWA:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SET_CALLFWDING:
        ...
        return FSM_SetCallFwding (req, event, data, datalen);

    case ME_GET_CALLFWDING:
        
        return FSM_GetCallFwding (req, event, data, datalen);

    case ME_SET_CALLBARRING:
        return FSM_SetCallBarring (req, event, data, datalen);

    case ME_GET_CALLBARRING:
        return FSM_GetCallBarring (req, event, data, datalen);

    case ME_SMS_SEND:
        return FSM_SMSSend (req, event, data, datalen);

    case ME_SMS_READ:
        return FSM_SMSRead (req, event, data, datalen);

    case ME_SMS_DELETE:
        return FSM_SMSDelete (req, event, data, datalen);

    case ME_SMS_LIST:
        return FSM_SMSList (req, event, data, datalen);
    ...
    case ME_SMS_ACK:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CNMA\r", -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_SMS_SET_SCA:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            if ( START == event ) {
                retval = MAT_Send (req->queue+1, "AT+CSCS=\"GSM\"\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
            ...
            if ( DATAIN == event ) {
                ...
                KillTimer( NULL, gQueue[req->queue].TimerID );
                if ( retval == FRC_OK) {
                    sprintf (ATCmd, "AT+CSCA=\"%s\"\r", req->cmddata);
                    retval = MAT_Send (req->queue+1, ATCmd, strlen(ATCmd));
                    gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                    req->stat = FSM_STATE_STEP2;
                }
    ...
    case ME_SMS_GET_SCA:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            ...
            if ( DATAIN == event ) {
                ...
                KillTimer( NULL, gQueue[req->queue].TimerID );
                if ( retval == FRC_OK) {
                    retval = MAT_Send (req->queue+1, "AT+CSCA?\r", -1 );
                    gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                    req->stat = FSM_STATE_STEP2;
                }
                else {
            ...
            if ( DATAIN == event ) {
                if ( strstr( data, "+CSCA:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
    ...
    case ME_SMS_SETINDSTATUS:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CSMS=1\r", -1);
                MAT_Send (0, "AT+CNMI=0,0,0,0,1\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
        ...
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                if ( retval == FRC_OK )
                {
                    ...
                    mt = ((PIND_STATUS)(req->cmddata))->Mt;
                    bm = ((PIND_STATUS)(req->cmddata))->Bm;
                    ds = ((PIND_STATUS)(req->cmddata))->Ds;
        ...
        else if ( FSM_STATE_STEP2 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                if ( retval == FRC_OK )
                {
                    retval = MAT_Send (req->queue+1, "AT&W\r", -1);
                    gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                    req->stat = FSM_STATE_STEP3;
                }
                else
    ...
    case ME_SMS_GETINDSTATUS:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CNMI?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CNMI:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SET_CELLBROADCAST:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                mode = ((PCELL_BROADCAST)(req->cmddata))->mode;
                ids = ((PCELL_BROADCAST)(req->cmddata))->ids;
                code = ((PCELL_BROADCAST)(req->cmddata))->code;
    ...
    case ME_GET_CELLBROADCAST:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CSCB?\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr (data, "+CSCB:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SMS_MEMCOUNT:
        ...
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CPMS:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_PBOOK_READ:
        return FSM_PBookRead (req, event, data, datalen);
    ...
    case ME_PBOOK_WRITE:
        {
            ...
            if ( FSM_STATE_INIT == req->stat )
            {
                if ( START == event ) {
                    ...
                    {
                    case PBOOK_SM:
                        retval = MAT_Send (req->queue+1, "AT+CPBS=\"SM\"\r", -1 );
                        break;
                    case PBOOK_ON:
                        retval = MAT_Send (req->queue+1, "AT+CPBS=\"ON\"\r", -1);
                        break;
                    case PBOOK_FD:
                        retval = MAT_Send (req->queue+1, "AT+CPBS=\"FD\"\r", -1);
                        break;
                    default:
                        goto ERROR_HANDLER;
                    }
                ...
                if ( DATAIN == event )
                {
                    ...
                    KillTimer (NULL, gQueue[req->queue].TimerID);
                    if ( retval == FRC_OK ) {
                        index = ((PPHONEBOOK)(req->cmddata))->Index;
                        PhoneNum = ((PPHONEBOOK)(req->cmddata))->PhoneNum;
                        Text = ((PPHONEBOOK)(req->cmddata))->Text;
    ...
    case ME_PBOOK_MEMCOUNT:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                case PBOOK_SM:
                    ...
                    break;
                case PBOOK_LD:
                    retval = MAT_Send (req->queue+1, "AT+CPBS=\"LD\"\r", -1);
                    break;
                case PBOOK_MC:
                    retval = MAT_Send (req->queue+1, "AT+CPBS=\"MC\"\r", -1);
                    break;
                case PBOOK_RC:
                    retval = MAT_Send (req->queue+1, "AT+CPBS=\"RC\"\r", -1);
                    break;
                case PBOOK_FD:
                    retval = MAT_Send (req->queue+1, "AT+CPBS=\"FD\"\r", -1);
                    break;
        ...
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                if ( retval == FRC_OK )
                {
                    retval = MAT_Send (req->queue+1, "AT+CPBS?\r", -1 );
                    gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                    req->stat = FSM_STATE_STEP2;
                }
                else
        ...
        else if ( FSM_STATE_STEP2 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CPBS:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SET_LOCK:
        ...
        return FSM_SetPhoneLock (req, event, data, datalen);

    case ME_GET_LOCK:
        
        return FSM_GetPhoneLock (req, event, data, datalen);

    case ME_PASS_CHANGE:

        return FSM_ChangePassword (req, event, data, datalen);
    ...
    case ME_PASS_VALID:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                password = req->cmddata;
                
                if ( req->cmddata_len == (int)strlen(password) )
                    newpass = NULL;
                else
                    newpass = &password[strlen(password)+1];

                if ( req->arg1 == PUK1 || req->arg1 == PUK2 )
                {
                    if ( newpass == NULL )
                        goto ERROR_HANDLER;
                }

                if ( req->arg1 == PIN2 || req->arg1 == PUK2 )
                ...
                retval = MAT_Send (req->queue+1, ATCmd, strlen(ATCmd) );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, 10000, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
    ...
    case ME_GET_PASS:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CPIN?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CPIN:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_PASS_REMAIN:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^SPIC\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "^SPIC:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                case MODE_MANUAL:
                    ...
                    sprintf( ATCmd, "AT+COPS=4,2,%s\r", req->cmddata );
                    break;
    ...
    case ME_GET_OPRATOR:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                ...
                case FORMAT_LONGALPHA:
                    retval = MAT_Send (req->queue+1, "AT+COPS=3,0\r", -1 );
                    break;
                case FORMAT_NUMERIC:
                    retval = MAT_Send (req->queue+1, "AT+COPS=3,2\r", -1 );
                    break;
                default:
                    goto ERROR_HANDLER;
                }
        ...
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                ...
                if ( retval == FRC_OK)
                {
                    retval = MAT_Send (req->queue+1, "AT+COPS?\r", -1 );
                    gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                    req->stat = FSM_STATE_STEP2;
                }
                else
        ...
        else if ( FSM_STATE_STEP2 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+COPS:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SRCH_OPRATOR:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            ...
            {
                retval = MAT_Send (req->queue+1, "AT+COPS=?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, 60000, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            ...
            {
                retval = MAT_Send (req->queue+1, "B\r", -1);
                KillTimer (NULL, gQueue[req->queue].TimerID);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP2;
            }
    ...
    case ME_GET_IMEI:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CGSN\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_GET_IMSI:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CIMI\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_SET_CLOCK:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                sprintf (ATCmd, "AT+CCLK=\"%s\"\r", req->cmddata);
    ...
    case ME_GET_CLOCK:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CCLK?\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr(data, "+CCLK:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SET_VOLUME:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                if ( req->arg1 < 0 || req->arg1 > 4 )
                    goto ERROR_HANDLER;

                sprintf (ATCmd, "AT+CLVL=%d\r", req->arg1);
    ...
    case ME_GET_VOLUME:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CLVL?\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr(data, "+CLVL:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_SEND_DTMF:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                sprintf (ATCmd, "AT+VTS=%c\r", req->arg1);
    ...
    case ME_FAC_SETTING:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT&F\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_RESET:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CFUN=1,1\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_SLEEP:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CFUN=7\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_SHUTDOWN:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^SMSO\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_GET_SIGNAL:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CSQ\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CSQ:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_GET_BATTERY:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^SBV\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "^SBV:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_GETCCM:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            if ( START == event ) {
                retval = MAT_Send (req->queue+1, "AT+CAOC\r", -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
            ...
            if ( DATAIN == event ) {
                if ( strstr (data, "+CAOC:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
    ...
    case ME_GETACM:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            if ( START == event ) {
                retval = MAT_Send (req->queue+1, "AT+CACM?\r", -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
            ...
            if ( DATAIN == event ) {
                if ( strstr (data, "+CACM:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
    ...
    case ME_GETACMAX:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            if ( START == event ) {
                retval = MAT_Send (req->queue+1, "AT+CAMM?\r", -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
            ...
            if ( DATAIN == event ) {
                if ( strstr (data, "+CAMM:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
    ...
    case ME_RESETACM:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            if ( START == event ) {
                char * pin2 = req->cmddata;
                sprintf (ATCmd, "AT+CACM=\"%s\"\r", pin2);
                retval = MAT_Send (req->queue+1, ATCmd, -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_GETPUC:
        if ( FSM_STATE_INIT == req->stat ) {
            ...
            if ( DATAIN == event ) {
                ...
                KillTimer (NULL, gQueue[req->queue].TimerID);
                if ( retval == FRC_OK) {
                    retval = MAT_Send (req->queue+1, "AT+CPUC?\r", -1);
                    gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                    req->stat = FSM_STATE_STEP2;
                }
                else {
            ...
            if ( DATAIN == event ) {
                if ( strstr (data, "+CPUC:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
                }
    ...
    case ME_SETPUC:
        ...
        if ( FSM_STATE_INIT == req->stat ) {
            ...
            if ( DATAIN == event ) {
                ...
                KillTimer( NULL, gQueue[req->queue].TimerID );
                if ( retval == FRC_OK) {
                    PME_PUCINFO puc = (PME_PUCINFO)req->cmddata;
                    char * pin2 = &(((char *)(req->cmddata))[sizeof(*puc)]);
    ...
    case ME_SETALARM:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            ...
            {
                char timestr[40];
                ME_ALARMTIME* time = (ME_ALARMTIME*)req->cmddata;
    ...
    case ME_GETALARM:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT+CALA?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
        else if ( FSM_STATE_STEP1 == req->stat )
        {
            if ( DATAIN == event )
            {
                if ( strstr( data, "+CALA:") == data && ME_MemGetOffset (req->queue) == 0 )
                {
                    data[datalen-1] = '\n';
                    ME_MemAssign (req->queue, data, datalen);
                    return 0;
    ...
    case ME_AUDIO:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                if ( req->arg1 == ME_AUDIO_HEADSET)
                    retval = MAT_Send (req->queue+1, "AT^SNFS=3\r", -1);
                else
                    retval = MAT_Send (req->queue+1, "AT^SNFS=4\r", -1);
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_MONI_SERV:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^MONI;^SMONC\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
    ...
    case ME_MONI_NEIB:
        ...
        if ( FSM_STATE_INIT == req->stat )
        {
            if ( START == event )
            {
                retval = MAT_Send (req->queue+1, "AT^MONP\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP1;
            }
        }
...
static int FSM_SpecialCall (MEREQUEST * req, int event, char * data, int datalen)
{
    ...
    case FSM_STATE_INIT:
        ...
        if ( event == START ) {
            char * ssCode = req->cmddata;
            sprintf (cmd, "ATD%s;\r", ssCode);
            MAT_Send (req->queue+1, cmd, strlen(cmd));
            gQueue[req->queue].TimerID = SetTimer (NULL, 0, SS_TIMEOUT, (TIMERPROC)ME_TimerProc);
            if ( req->arg1 )
                req->stat = FSM_STATE_CALLCONTROL;
            else
                req->stat = FSM_STATE_SSCONTROL;
        }
    ...
    case FSM_STATE_SSCONTROL:
        ...
        if ( event == DATAIN ) {
            ...
            if ( retval < 0 )
                return retval;
            if ( req->error == ME_RS_SUCCESS ) {
                int size = 0;
                KillTimer (NULL, gQueue[req->queue].TimerID);
                req->result = ME_MemGetPntr (0);
                if ( req->arg2 < 0 ) {
                    req->errcode = SSUNKNOWN;
                    req->result_len = -1;
                }
                else {
                    req->errcode = req->arg2;
                    switch (req->arg2) {
                    case SSCLIP:
                    ...
                    req->result_len = size * req->arg3;
                }
                req->stat = FSM_STATE_DONE;
                return 0;
            }
            if ( req->error == ME_RS_FAILURE ) {
               KillTimer (NULL, gQueue[req->queue].TimerID);
               req->result = NULL;
               req->result_len = -1;
               req->stat = FSM_STATE_DONE;
...
static int FSM_USSD (int event, char* data, int datalen)
{
    ...
    case USSD_IDLE:
        ...
        if ( event == REQUEST ) {
            sprintf (cmd, "ATD%s;\r", data);
            MAT_Send (0, cmd, strlen(cmd));
...
static int FSM_SetCallFwding (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    if ( FSM_STATE_INIT == req->stat )
    {
        if ( START == event )
        {
            reason = ((PCALL_FWD)req->cmddata)->Reason;
            enable = ((PCALL_FWD)req->cmddata)->bEnable;
            Class = ((PCALL_FWD)req->cmddata)->Class;
            PhoneNum = ((PCALL_FWD)req->cmddata)->PhoneNum;
            time = ((PCALL_FWD)req->cmddata)->Time;
...
static int FSM_GetCallFwding (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    else if ( FSM_STATE_STEP1 == req->stat )
    {
        if ( DATAIN == event )
        {
            if ( strstr( data, "+CCFC:") == data && ME_MemGetOffset (req->queue) == 0 )
            {
                data[datalen-1] = '\n';
                ME_MemAssign (req->queue, data, datalen);
                return 0;
...
static int FSM_GetCallBarring (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    else if ( FSM_STATE_STEP1 == req->stat )
    {
        if ( DATAIN == event )
        {
            if ( strstr( data, "+CLCK:") == data && ME_MemGetOffset (req->queue) == 0 )
            {
                data[datalen-1] = '\n';
                ME_MemAssign (req->queue, data, datalen);
                return 0;
...
static int FSM_SMSSend (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    case FSM_STATE_INIT:
        ...
        if ( START == event ) {
            retval = MAT_Send (req->queue+1, "AT+CMGF?\r", -1 );
            gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
            req->stat = FSM_STATE_STEP1;
        }
    ...
    case FSM_STATE_STEP1:
        ...
        if ( DATAIN == event ) {
            if ( strstr (data, "+CMGF:") == data && ME_MemGetOffset (req->queue) == 0 ) {
                data[datalen-1] = '\n';
                ME_MemAssign (req->queue, data, datalen);
                return 0;
            }
            ...
            KillTimer (NULL, gQueue[req->queue].TimerID);
            temp = ME_MemGetPntr (req->queue);
            retval = ME_GetIntSection (temp, "+CMGF:", 1);
            if ( retval == 1 ) {
                retval = MAT_Send (req->queue+1, "AT+CMGF=0\r", -1);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, DFT_TIMEOUT, ME_TimerProc);
                req->stat = FSM_STATE_STEP2;
            }
            else if ( retval == 0 ) {
                int len;
                PSMS_SEND pSend = req->cmddata;
                memset (PDUCode, 0, ME_SMS_PDU_LEN);
                len = ME_TranStr_Code (pSend, PDUCode, ME_SMS_PDU_LEN);
                if ( len <= 0 )
                    goto ERROR_HANDLER;
                sprintf (ATCmd, "AT+CMGS=%d\r", len);
                req->cmddata = malloc (strlen(PDUCode)+1);
                strcpy (req->cmddata, PDUCode);
                req->cmddata_len = strlen (PDUCode);
    ...
    case FSM_STATE_STEP3:
        ...
        if ( DATAIN == event ) {
            if ( datalen == 1 && data[0] == '>' ) {
                KillTimer( NULL, gQueue[req->queue].TimerID );
                retval = MAT_Send (req->queue+1, req->cmddata, req->cmddata_len);
                gQueue[req->queue].TimerID = SetTimer (NULL, 0, 25000, ME_TimerProc);
                req->stat = FSM_STATE_STEP4;
            }
            else {
                retval = GetResultErrorCode (req, data, datalen);
                if ( retval < 0 )
    ...
    case FSM_STATE_STEP4:
        ...
        if ( DATAIN == event ) {
			...
			escape = 0x1B;
			free( req->cmddata);
            retval = MAT_Send (req->queue+1, &escape, 1);
    		gQueue[req->queue].TimerID = SetTimer (NULL, 0, 4000, ME_TimerProc);
            req->arg1 = ME_RS_USERSTOP;
			req->stat = FSM_STATE_STEP5;
        }
        else if ( TIMEOUT == event ) {
			char escape;
			escape = 0x1B;
...
static int FSM_SMSRead (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    if ( FSM_STATE_INIT == req->stat )
    {
        if ( START == event )
        {
            ...
            {
            case SMS_MEM_SM:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"SM\"\r", -1 );
                break;
            case SMS_MEM_ME:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"ME\"\r", -1 );
                break;
            case SMS_MEM_MT:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"MT\"\r", -1 );
                break;
            default:
                goto ERROR_HANDLER;
            }
    ...
    else if ( FSM_STATE_STEP1 == req->stat )
    {
        if ( DATAIN == event )
        {
            ...
            else if ( retval == FRC_CMSERROR && req->errcode == 522)
            {
                switch ( req->arg2 )
                {
                case SMS_MEM_SM:
    ...
    else if ( FSM_STATE_STEP2 == req->stat )
    {
        if ( DATAIN == event )
        {
            ...
            if ( retval == FRC_OK )
            {
                ...
                {
                    if ( req->arg1 )
                        sprintf( ATCmd, "AT^SMGR=%d\r", req->arg3 );
                    else
                        sprintf( ATCmd, "AT+CMGR=%d\r", req->arg3 );
    ...
    else if ( FSM_STATE_STEP4 == req->stat )
    {
        if ( DATAIN == event )
        {
    ...
    else if ( FSM_STATE_STEP5 == req->stat )
    {
        if ( DATAIN == event )
        {
            data[datalen-1] = '\n';
...
static int FSM_SMSDelete (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    if ( FSM_STATE_INIT == req->stat )
    {
        if ( START == event )
        {
            ...
            {
            case SMS_MEM_SM:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"SM\",\"SM\"\r", -1 );
                break;
            case SMS_MEM_ME:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"ME\",\"ME\"\r", -1 );
                break;
            case SMS_MEM_MT:
                retval = MAT_Send (req->queue+1, "AT+CPMS=\"MT\",\"MT\"\r", -1 );
                break;
            default:
                goto ERROR_HANDLER;
            }
    ...
    else if ( FSM_STATE_STEP1 == req->stat )
    {
        if ( DATAIN == event )
        {
            ...
            if ( retval == FRC_OK)
            {
                sprintf( ATCmd, "AT+CMGD=%d\r", req->arg2 );
                retval = MAT_Send (req->queue+1, ATCmd, strlen(ATCmd) );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP2;
            }
    ...
    else if ( FSM_STATE_STEP2 == req->stat )
    {
        if ( DATAIN == event )
        {
            ...
            else if ( retval == 0 )
            {
                ...
                case SMS_UNREAD:
                    if ( req->arg1 )
                        retval = MAT_Send (req->queue+1, "AT^SMGL=0\r", -1 );
                    else
                        retval = MAT_Send (req->queue+1, "AT+CMGL=0\r", -1 );
                    break;

                case SMS_READED:
                    if ( req->arg1 )
                        retval = MAT_Send (req->queue+1, "AT^SMGL=1\r", -1 );
                    else
                        retval = MAT_Send (req->queue+1, "AT+CMGL=1\r", -1 );
                    break;

                case SMS_UNSEND:
                    if ( req->arg1 )
                        retval = MAT_Send (req->queue+1, "AT^SMGL=2\r", -1 );
                    else
                        retval = MAT_Send (req->queue+1, "AT+CMGL=2\r", -1 );
                    break;

                case SMS_SENDED:
                    if ( req->arg1 )
                        retval = MAT_Send (req->queue+1, "AT^SMGL=3\r", -1 );
                    else
                        retval = MAT_Send (req->queue+1, "AT+CMGL=3\r", -1 );
                    break;

                case SMS_ALL:
                    if ( req->arg1 )
                        retval = MAT_Send (req->queue+1, "AT^SMGL=4\r", -1 );
                    else
                        retval = MAT_Send (req->queue+1, "AT+CMGL=4\r", -1 );
                    break;

                default:
                    goto ERROR_HANDLER;
                }
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, 16000, ME_TimerProc );
                req->stat = FSM_STATE_STEP4;
            }
            else
                goto ERROR_HANDLER;
...
static int FSM_PBookRead (MEREQUEST * req, int event, char* data, int datalen)
{
    ...
    else if ( FSM_STATE_STEP1 == req->stat )
    {
        if ( DATAIN == event )
        {
            ...
            if ( retval == FRC_OK)
            {
                retval = MAT_Send (req->queue+1, "AT+CSCS?\r", -1 );
                gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                req->stat = FSM_STATE_STEP2;
            }
            else
    ...
    else if ( FSM_STATE_STEP2 == req->stat )
    {
        ...
        {
            char result[20];
            if ( strstr( data, "+CSCS:") == data && ME_MemGetOffset (req->queue) == 0 )
            {
                data[datalen-1] = '\n';
                ME_MemAssign (req->queue, data, datalen);
                return 0;
            ...
            if ( retval == FRC_OK )
            {
                ...
                {
                    retval = MAT_Send (req->queue+1, "AT+CSCS=\"UCS2\"\r", -1 );
                    gQueue[req->queue].TimerID = SetTimer( NULL, 0, DFT_TIMEOUT, ME_TimerProc );
                    req->stat = FSM_STATE_STEP3;
                }
                else
                {
                    if ( req->arg3 == 0 )
                        sprintf( ATCmd, "AT+CPBR=%d\r", req->arg2 );
                    else if ( req->arg3 > req->arg2 )
                        sprintf( ATCmd, "AT+CPBR=%d,%d\r", req->arg2, req->arg3 );
                    else
                        goto ERROR_HANDLER;
                    
                    retval = MAT_Send (req->queue+1, ATCmd, strlen(ATCmd) );
                    gQueue[req->queue].TimerID = SetTimer( NULL, 0, 30000, ME_TimerProc );
                    req->stat = FSM_STATE_STEP4;
                }
            }
            else
...
static int GetResultErrorCode (MEREQUEST * req, char* data, int datalen)
{
    ...
    case FRC_CMEERROR:
        ...
        req->errcode = atoi( &data[strlen(FRC_String[FRC_CMEERROR])] );
        break;
    ...
    case FRC_CMSERROR:
        ...
        req->errcode = atoi( &data[strlen(FRC_String[FRC_CMSERROR])] );
        break;

    case FRC_BUSY:
        req->error = ME_RS_BUSY;
        req->errcode = 0;
        break;

    case FRC_NODIALTONE:
        req->error = ME_RS_NODIALTONE;
        req->errcode = 0;
        break;

    case FRC_NOCARRIER:
        req->error = ME_RS_NOCARRIER;
        req->errcode = 0;
        break;

    case FRC_CONNECT:
        req->error = ME_RS_CONNECT;
        req->errcode = 0;
        break;
    }
...
void GetResultLength (MEREQUEST* pRequest)
{
    unsigned int len = ME_MemGetOffset (pRequest->queue);
    char *str, *begin, *end;
    ...
    str = ME_MemGetPntr (pRequest->queue);
    len = ME_MemGetOffset (pRequest->queue);
